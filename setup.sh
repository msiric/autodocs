...
cmd_upgrade() {
  ...
  # Detect REPO_DIR from existing rendered sync script
  local repo_dir=""
  if [ -f "$output_dir/autodocs-sync.sh" ]; then
    repo_dir=$(grep '^REPO_DIR=' "$output_dir/autodocs-sync.sh" 2>/dev/null | head -1 | cut -d'"' -f2)
  fi
  if [ -z "$repo_dir" ]; then
    read -rp "Repo directory path: " repo_dir
  ...
  echo "Upgrading autodocs in $output_dir..."
  echo "  Repo: $repo_dir"
  echo "  Config: preserved (not modified)"

  # Re-render prompts from latest templates (only OUTPUT_DIR needs substitution;
  # feature_name and owner.name are read from config.yaml by the LLM at runtime)
  echo "  Updating prompts..."
  envsubst '${OUTPUT_DIR}' < "$TEMPLATES_DIR/sync-prompt.md" > "$output_dir/sync-prompt.md"
...
# Compute date range
dates = sorted(set(e['ts'][:10] for e in entries))
print(f'=== autodocs metrics ({len(dates)} days, {len(entries)} entries) ===')
print(f'Date range: {dates[0]} to {dates[-1]}')
print()

# Per-stage success rates
stages = ['sync', 'drift', 'suggest', 'apply']
print('Stage success rates:')
for stage in stages:
    stage_entries = [e for e in entries if e['call'] == stage]
    if not stage_entries:
        continue
    ok = sum(1 for e in stage_entries if e['status'] == 'success')
    total = len(stage_entries)
    pct = ok / total * 100 if total else 0
    bar = '#' * int(pct / 5) + '.' * (20 - int(pct / 5))
    print(f'  {stage:8s} [{bar}] {pct:5.1f}%  ({ok}/{total})')
print()

# Match rate trend (last 10)
match_entries = [e for e in entries if e['call'] == 'match-rate'][-10:]
if match_entries:
    print('Match rate (last 10):')
    for e in match_entries:
        print(f'  {e[\"ts\"][:10]}  {e[\"status\"]}')
    print()

# Last 5 failures
failures = [e for e in entries if e['status'] == 'failed'][-5:]
if failures:
    print('Recent failures:')
    for e in failures:
        print(f'  {e[\"ts\"]}  {e[\"call\"]:8s}  exit {e.get(\"rc\", \"?\")}')
    print()

# Runs per day (last 7 days)
cutoff = (datetime.utcnow() - timedelta(days=7)).strftime('%Y-%m-%d')
recent_sync = [e for e in entries if e['call'] == 'sync' and e['ts'][:10] >= cutoff]
if recent_sync:
    by_day = Counter(e['ts'][:10] for e in recent_sync)
...
case "${1:-}" in
  ...
  generate)
    ...
    local_output="${2:-$local_repo/.autodocs}"
    mkdir -p "$local_output"
    python3 "$SCRIPT_DIR/scripts/generate.py" "$local_repo" "$local_output" "${@:3}"
    exit 0
    ;;
  catchup)
    shift
  ...
  "")      ;; # no args = full setup
  -*)      ;; # other flags
  *)       echo "Usage: setup.sh [--quick] | team | docs | paths | config | analyze | generate | status | metrics | upgrade | catchup"; exit 1 ;;
esac
...
OWNER_NAME="" ; OWNER_EMAIL="" ; OWNER_USERNAME="" ; OWNER_ADO_ID=""
...
# Prompt for platform-specific username (distinct from repo owner)
case "$PLATFORM" in
  github)
    DETECTED_USERNAME=$(gh api user --jq .login 2>/dev/null || true)
    if [ -n "$DETECTED_USERNAME" ]; then
      echo "Detected GitHub username: $DETECTED_USERNAME"
      if confirm "Correct?"; then
        OWNER_USERNAME="$DETECTED_USERNAME"
      fi
    fi
    [ -z "$OWNER_USERNAME" ] && read -rp "Your GitHub username: " OWNER_USERNAME
    ;;
  gitlab)
    read -rp "Your GitLab username: " OWNER_USERNAME
    ;;
  bitbucket)
    read -rp "Your Bitbucket username: " OWNER_USERNAME
    ;;
  ado)
    read -rp "Your ADO ID (GUID): " OWNER_ADO_ID
    OWNER_USERNAME="$OWNER_ADO_ID"
    ;;
esac
...
if [ -f "$CONFIG_FILE" ]; then
  ...
  case "$PLATFORM" in
    github)    OWNER_USERNAME_FIELD="  github_username: \"${OWNER_USERNAME}\"" ;;
    gitlab)    OWNER_USERNAME_FIELD="  gitlab_username: \"${OWNER_USERNAME}\"" ;;
    bitbucket) OWNER_USERNAME_FIELD="  bitbucket_username: \"${OWNER_USERNAME}\"" ;;
...
export OUTPUT_DIR REPO_DIR SCHEDULE_HOUR
